...
export const githubFilesAddedTrigger: TriggerConfig = {
  id: 'github_files_added',
  name: 'GitHub Files Added',
  provider: 'github',
  description: 'Trigger workflow when new files are added to the repository',
  version: '1.0.0',
  icon: GithubIcon,
  ...
  subBlocks: [
    ...
    },
    {
      ...
      description:
        'Optional glob pattern to filter files. Supports wildcards. Leave empty to match all new files.',
      required: false,
      mode: 'trigger',
      condition: {
        field: 'selectedTriggerId',
    ...
    },
    {
      ...
      defaultValue: [
        ...
        '<em>Note: This trigger only fires when new files are added to the repository.</em>',
      ]
        .map(
          (instruction, index) =>
  ...
  outputs: {
    ...
    added_files: {
      type: 'array',
      description: 'Array of all file paths that were added across all commits',
    },
    added_files_count: {
      type: 'number',
      description: 'Total number of files added',
    },
    commits: {
      type: 'array',
      description: 'Array of commits that added files',
      items: {
